...
const Works2 = () => {
  ...
  const handlePortfolioData = (id) => {
    const find = info.portfolio.find((item) => item?.id === id);
    setSingleData(find);
    setIsOpen(true);
  };
  ...
  const [data, setData] = useState(info.portfolio);

  // fillter portfilo data
  const handleData = (text) => {
    if (text === 'All') {
      setData(info.portfolio);
    } else {
      const findData = info.portfolio.filter((item) => item.tag === text);
      setData(findData);
    }
  };
  ...
  return (
    <>
      ...
      <ul className='mt-[40px] flex w-full justify-start md:justify-end flex-wrap font-medium pb-12'>
        <li
          className={`${
            filter === 'All' ? 'text-[#FA5252]' : 'fillter-btn '
          } mr-4 md:mx-4`}
        ...
        <li
          className={`${
            filter === 'Web' ? 'text-[#FA5252]' : 'fillter-btn'
          } mr-4 md:mx-4`}
        ...
        <li
          className={`${
            filter === 'Automatization' ? 'text-[#FA5252]' : 'fillter-btn'
          } mr-4 md:mx-4`}
          onClick={() => handleSearch('Automatization')}
        >
          Automatization
        </li>
        <li
          className={`${
            filter === 'Lib' ? 'text-[#FA5252]' : 'fillter-btn ml-0'
          } mr-4 md:mx-4`}
          onClick={() => handleSearch('Lib')}
        >
          Lib
        </li>
        <li
          className={`${
            filter === 'Studies/Ideas' ? 'text-[#FA5252]' : 'fillter-btn'
          }  `}
          onClick={() => handleSearch('Studies/Ideas')}
        >
          Studies/Ideas
        </li>
      </ul>
            ...
            <div className='grid grid-cols-1 lg:grid-cols-2 my-6'>
              <div className='space-y-2'>
                ...
                <p className='dark:text-white flex items-center text-[15px]  sm:text-lg '>
                  ...
                  Stack :&nbsp;
                  <span className='font-medium '>{singleData?.stack}</span>
                </p>
              </div>
